\subsection{Setup} \label{sec:05-setup}
\begin{foldable} % Datasets and models
  We evaluate on two linguistic tasks: AG News ($N=120$\,K), IMDb ($N=25$\,K), SST-2 ($N=67$\,K) for classification; MNLI-m ($N=393$\,K) and QQP ($N=364$\,K) for natural language inference (NLI).
...
\begin{table}[ht]
  ...
  \begin{threeparttable}
    ...
    \begin{tabular}{l l c c c}
      ...
      & Random          & 20/cls & \meanstd{52.40}{3.12}   & \meanstd{59.10}{3.80} \\
...
\begin{foldable} % NLI
  ...
  The LR gap ($+8.5$ on MNLI-m, $+7.6$ on QQP relative to Random) is particularly informative: Siamese LR has no capacity to compensate for a poor corpus --- every percentage point of accuracy here must come from the quality of $\tilde{\mathcal{D}}$ itself.
  ...
  This is a harder coverage requirement than single-sentence classification, and it points to two compounding limitations: the 20/cls budget leaves only $\approx\!60$ pairs for MNLI-m, and the synthetic pool must jointly cover all premise--hypothesis relationship types.
  Either could be the binding bottleneck; the gap narrows the hypothesis space to pool generation and budget, rather than scoring or selection.
