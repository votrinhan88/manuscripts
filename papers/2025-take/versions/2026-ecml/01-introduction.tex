...
\begin{foldable} % DD --> Text DD
  Dataset distillation (DD), first proposed by~\cite{wang2018dataset}, replaces a large dataset $\mathcal{D}$ with a smaller surrogate $\tilde{\mathcal{D}} \ll \mathcal{D}$ such that a model trained on $\tilde{\mathcal{D}}$ performs comparably to one trained on $\mathcal{D}$.
  ...
  Early DD methods work in image space, optimizing synthetic pixels via meta-learning, gradient or trajectory matching~\cite{wang2018dataset,zhao2020dataset,cazenavette2022dataset}.
...
\begin{foldable} % Baseline text DD methods --> gap --> desiderata
  ...
  These gaps motivate three desiderata for TAKE:{
    ...
    and (3)~\emph{human-readable output} --- a distilled corpus that is directly auditable and transferable across architectures.
  }
\end{foldable}

\begin{foldable} % Our method TAKE and contributions
  We propose \textbf{Trajectory-Aware Knowledge Estimation} (\textbf{TAKE}), the first text DD method to satisfy all three desiderata.
  ...
  \begin{itemize}
    ...
    \item {
        \textbf{Empirical:} TAKE matches or exceeds prior text DD methods across six language benchmarks at extreme compression (0.1\% or 20 instances/class), producing human-readable distilled corpora that generalize across diverse backbone families.
      }
  \end{itemize}
\end{foldable}
