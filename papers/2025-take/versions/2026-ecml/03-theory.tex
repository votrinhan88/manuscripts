...
\begin{foldable} % DD --> DM --> RDM
    ...
    \end{equation}
    where $F(\theta_0;\, \tilde{\mathcal{D}}, \eta, T)$ denotes the model obtained by running $T$ steps of gradient descent with learning rate $\eta$ on $\tilde{\mathcal{D}}$ from initialisation $\theta_0$.
    ...
    Since $\nabla_\theta \mathbb{E}_{z \sim P}[\ell(z;\theta)] = \mathbb{E}_{z \sim P}[\nabla_\theta \ell(z;\theta)]$, bounding the discrepancy between $P_{\tilde{\mathcal{D}}}$ and $P_{\mathcal{D}}$ in a task-relevant feature space directly bounds the gradient mismatch --- making distribution matching a tractable surrogate for~\eqref{eq:dataset-distillation}.
...
\begin{foldable} % Uniform DM --> RDM
    ...
    Influence self-scores (\S\ref{sec:02-related-influence}) are the natural starting point.
...
\subsection{Two Failures of the Naive Pipeline} \label{sec:03-theory-failures}
...
\begin{foldable} % Proposition 1: HSB
    ...
    Correcting it requires integrating gradient signals across the training trajectory (\S\ref{sec:04-method-gradient}).
...
\begin{foldable} % Two open problems
    ...
    \begin{enumerate}
        \item \textbf{Biased weighting.} Estimate $w_n$ by integrating gradient signals across trajectory $\{\theta_t\}_{t=0}^{T-1}$, reducing $\Delta_\textup{HSB}$---which we will address in \S\ref{sec:04-method-gradient} (formalised as Proposition~2; Appendix~A.2).
        \item \textbf{Independent selection.} Solve \eqref{eq:reweighted-distribution-matching} directly for a metric $d$ that jointly optimises coverage of $P^w$---which we will address in \S\ref{sec:04-method-ot}.
    \end{enumerate}
\end{foldable}