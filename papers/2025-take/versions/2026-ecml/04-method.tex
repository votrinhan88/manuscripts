...
\begin{foldable}
  A decreasing kernel is the natural choice: early checkpoints capture easy samples fitting rapidly, mid checkpoints offer the strongest signal, and late checkpoints see the gap narrow as memorization sets in~\cite{arpit2017closer}.
...
\begin{foldable}
  \begin{proposition}[Trajectory Integration Reduces HSB] \label{prop:traj}
    Under a smooth loss and bounded gradient noise, $\kappa_n$ produces a strictly smaller noisy-over-clean gap than the single last-checkpoint baseline $\kappa^\dagger_n = 1/I_{(n,T-1)}$:
    \begin{equation}
  ...
  \begin{proof}[Proof sketch (full proof: Appendix~A.2)]
    Define the noisy-over-clean gap $\Delta_t := \mathbb{E}_\textup{noisy}[1/I_{(n,t)}] - \mathbb{E}_\textup{clean}[1/I_{(n,t)}]$.
...
\begin{foldable} % Method wrap-up
  ...
  Algorithm~\ref{alg:take} consolidates the full pipeline.
\end{foldable}
