\subsection{Problem Setup and Framework Overview} \label{sec:04-setup}
\begin{foldable} % Setup + overview merged
	TAKE instantiates the RDM objective~\eqref{eq:reweighted-distribution-matching} by solving the two open problems from \S\ref{sec:03-theory-failures}: trajectory-aware knowledge estimation (\S\ref{sec:04-method-gradient}) yields task-informed weights $w_n$; discrete Optimal Transport~(OT) (\S\ref{sec:04-method-ot}) selects prototypes with optimal coverage of $P^w$.
...
\begin{foldable} % Motivation for trajectory-aware estimation
	The hard-sample bias (HSB) from \S\ref{sec:03-theory-failures} arises because single-checkpoint influence scores overemphasise noisy or difficult samples, whose gradients remain large at convergence.
	...
	We build $\kappa_n$ in two steps: constructing the influence matrix $I$ (\S\ref{sec:04-gradient-matrix}), then collapsing it into a scalar score via reciprocal reweighting and temporal discounting (\S\ref{sec:04-gradient-kappa}).
	The resulting $\kappa_n$ provides a task-informed, bias-corrected weight for $P^w$.
\end{foldable}


\subsubsection{Influence Matrix $I$} \label{sec:04-gradient-matrix}
\begin{foldable} % Definition and construction of I
	...
	The gradient norm recovers the structure needed to detect the HSB: well-fitted samples have near-zero $\nabla_\theta \ell$ by definition, so $I_{(n,t)}$ is naturally low for clean and moderate samples at convergence and persistently elevated for noisy ones (until memorisation onset, \S\ref{sec:04-gradient-kappa}).
...
\begin{foldable} % Inversion and temporal weighting to produce kappa
	...
	A decreasing kernel is necessary because gradient dynamics differ across the trajectory: early checkpoints capture easy samples fitting rapidly; mid checkpoints capture moderate and boundary samples still being learned; late checkpoints are dominated by memorisation, where noisy gradients finally drop and mimic well-fitted samples, contaminating scores~\cite{arpit2017closer}.
	A dataset of only easy samples produces a brittle model; the decreasing kernel accumulates signal across both the easy and moderate learning phases while discounting the late memorisation phase.
\end{foldable}

\begin{foldable}
	We define the memorisation onset $T_m$ as the first checkpoint at which the expected gradient norm of noisy samples begins to decrease:
	\begin{equation}
	...
	Setting the kernel decay to $\lambda^* = T / T_m$ concentrates mass on checkpoints $t \leq T_m$, where $\Delta_t \leq 0$, and exponentially suppresses the memorisation phase.
	...
	Normalised source weights $w_n = \kappa_n / \sum_{n'} \kappa_{n'}$ define the reweighted distribution $P^w$ and serve as input to the OT selection step (\S\ref{sec:04-method-ot}).
	...
	\begin{proof}[Proof sketch (full proof: Appendix~A.2)]
		...
		The left-hand side of~\eqref{eq:prop2} equals $\sum_t k(t)\Delta_t$.
		For $t \leq T_m$ (before memorisation onset): clean gradients decay so $\Delta_t \leq 0$, strictly at some $t^*$.
		Beyond $T_m$: noisy gradients also drop (memorisation), so $\Delta_{T-1} > 0$ — the last-checkpoint baseline is inflated by memorisation.
	...
	\end{proof}
	With HSB corrected by $\kappa_n$, the remaining question is how to select the distilled set $\tilde{\mathcal{D}}$ so that it covers the reweighted distribution $P^w$ — the subject of \S\ref{sec:04-method-ot}.
\end{foldable}

\subsection{Synthetic Candidate Pool Generation} \label{sec:04-pool}
...
\begin{foldable} % Distribution mismatch is beneficial
	...
	OT routes mass toward $P^w$ regardless of pool shift, so $\tilde{\mathcal{D}}$ approximates $P_{\mathcal{D}_\text{train}}$ given sufficient support coverage (Assumption~\ref{ass:coverage}).
...
\begin{foldable} % Formulation and entropic regularisation
	...
	We add entropic regularisation to obtain the strictly convex objective:
	\begin{equation}
	...
	The $K$ candidates with highest received mass $\sum_n \gamma^*_{nc}$ are selected as $\tilde{\mathcal{D}}$, directly addressing Open Problem~(b) from \S\ref{sec:03-theory-failures}.
	...
	\begin{proof}[Proof sketch (full proof: Appendix~A.3)]
		...
		Minimising~\eqref{eq:ot} directly minimises $W_1(P^w, P_{\tilde{\mathcal{D}}})$, controlling the dominant term.
	\end{proof}
...
\begin{algorithm}[H]
    ...
    Compute $\kappa_n \leftarrow \textstyle\sum_{t=0}^{T-1} k(t) / (I_{(n,t)} + \varepsilon)$ for all $n$ \hfill via Eq.~\eqref{eq:kappa} \\
    ...
    Solve $\gamma^* \leftarrow \mathrm{Sinkhorn}(C,\, w,\, \mathbf{1}/M,\, \varepsilon_\text{OT})$ \hfill via Eq.~\eqref{eq:ot} \\
