...
\begin{foldable}
    We evaluate DIPKD on extensive BBDFKD benchmarks, supplemented by ablation studies to validate its mechanics and practical effectiveness.
\end{foldable}

\subsection{Experimental setup} \label{ch4:ssec:04-experiments-setup}
\begin{foldable}
    We evaluate our method across 12 benchmarks, including 8 general-purpose datasets (MNIST~\cite{lecun1998gradient}, USPS~\cite{hull1994database}, SVHN~\cite{netzer2011reading}, FMNIST~\cite{xiao2017fashion}, CIFAR10, CIFAR100~\cite{krizhevsky2009learning}, Tiny-ImageNet~\cite{le2015tiny}, Imagenette~\cite{howard2020imagenette}), and 4 subsets from the medical dataset MedMNIST~\cite{yang2021medmnist}.
...
\subsection{Baselines} \label{ch4:ssec:04-experiments-baselines}
\begin{foldable}
    We compare our method DIPKD with the baselines:
    \begin{itemize}
...
\subsection{Standard experiments} \label{ch4:ssec:04-experiments-standard}
\begin{foldable}
    ...
    We report the distillation accuracy of all methods in Tab.~\ref{ch4:tab:standard-all}, where our method \textbf{DIPKD} consistently outperforms all baselines across 11/12 datasets, with significant margins on complex and domain-specific ones.
\end{foldable}
...
\begin{table}[ht]
    ...
    \begin{tabular}{c|c|c|c|c|c|c|c}
        \hline \textbf{Dataset} & \textbf{Setup} & \textbf{Teacher} & \textbf{NaiveKD} & \textbf{ZSDB3} & \textbf{IDEAL} & \textbf{DFHL-RS} & \textbf{DIPKD} \\
...
\begin{foldable}
    ...
    Notably, on high-resolution images like Imagenette, our DIPKD achieves a +21.31\% absolute gain over the strongest baseline, DFHL-RS.
    On domain-specific medical tasks, DIPKD outperforms baselines by at least +4\% to +12\%.
    We also notice that abundant classes represent an interesting boundary condition, where DIPKD remains highly competitive on CIFAR100 and approaches state-of-the-art on Tiny-ImageNet.
...
\begin{foldable}
    In terms of robustness, we observe that DIPKD has consistently low standard errors on most datasets, indicating strong stability across runs.
...
\subsection{Ablation studies} \label{ch4:ssec:04-experiments-ablation}
\begin{foldable}
...
\subsubsection{Contribution of components} \label{ch4:sssec:04-experiments-ablation-components}
...
\begin{table}[H]
    ...
    \end{tabular}
    \label{ch4:tab:abla-components}
\end{table}

\begin{foldable}
    We evaluate each component on MNIST and CIFAR10 to represent simple and complex benchmarks.
    Starting from NaiveKD, we sequentially add components and report student accuracy in Table~\ref{ch4:tab:abla-components}.
    Synthesis provides the largest gain ($+$2.27\% on MNIST, $+$64.18\% on CIFAR10), primarily driven by hierarchical noise.
    This is equivalent to the primer student $S_0$ trained on $\mathcal{X}_\mathcal{P}$, achieving 98\% and 78\% on MNIST and CIFAR10 (which already outperforms the baselines from Sec. \ref{ch4:ssec:04-experiments-standard}).
...
\subsubsection{Increasing synthetic dataset size} \label{ch4:sssec:04-experiments-ablation-nsamples}

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\columnwidth]{figures/40-research02/44-research02-experiments-abla-nsamples.pdf}
    \caption{Distillation accuracy (\%) by number of image priors $N$.}
    \label{ch4:fig:abla-nsamples}
\end{figure}

\begin{foldable}
    The relationship between synthetic data budget $N$ and KD performance mirrors the emergence of collective intelligence.
    While it is expected that KD performance improves with large $N$ (Fig.~\ref{ch4:fig:abla-nsamples}), we are interested in the optimal cut-off point, which indicates the \textbf{minimum quorum} required for the student to replicate the teacher's expertise.
...
\subsubsection{Embeddings analysis --- Synthesis} \label{ch4:sssec:04-experiments-ablation-embeddings-generation}
...
\begin{foldable}
    ...
    We visualize these embeddings in Fig.~\ref{ch4:fig:embeddings-universal}.
\end{foldable}

\begin{figure}[H]
    \centering
    \includegraphics[trim={120 0 0 0},clip,width=0.86\columnwidth]{figures/40-research02/44-research02-experiments-embeddings-objects.png}
    ...
    \label{ch4:fig:embeddings-universal}
\end{figure}
...
\begin{foldable}
    ...
    The statistics in Table~\ref{ch4:tab:abla-embeddings-coverage-objects} show that our image priors $\mathcal{X}_\mathcal{P}$ achieve significantly higher density, coverage, and recall at a small trade-off in precision, compared to noise sources $\mathcal{X}_{\mathcal{U}}$ and $\mathcal{X}_{\mathcal{N}}$.
...
\begin{table}[H]
    ...
    \label{ch4:tab:abla-embeddings-coverage-objects}
    \centering
...
\subsubsection{Embeddings analysis --- Contrast} \label{ch4:sssec:04-experiments-ablation-embeddings-contrast}
\begin{foldable}
    To analyze the Contrast phase's impact on diversity, we evaluate 50K image prior embeddings in Fig.~\ref{ch4:fig:embeddings-cifar10}.
    ...
    The distribution metrics in Table~\ref{ch4:tab:abla-embeddings-coverage-cifar10} confirm this with a significant increase in coverage and recall, with a trivial trade-off in density.
...
\begin{figure}[H]
    \centering
    \includegraphics[trim={120 0 0 0},clip,width=0.86\columnwidth]{figures/40-research02/44-research02-experiments-embeddings-cifar10.png}
    \caption{Feature embeddings of image priors $\mathcal{X}_\mathcal{P}$ before and after the Contrast phase.}
    \label{ch4:fig:embeddings-cifar10}
\end{figure}

\begin{table}[H]
    \caption{Distribution metrics (\%) during the Contrast phase.}
    \label{ch4:tab:abla-embeddings-coverage-cifar10}
    \centering
...
\subsubsection{Cross-architecture KD} \label{ch4:sssec:04-experiments-ablation-crossarch}
\begin{foldable}
    ...
    Table~\ref{ch4:tab:abla-crossarch} shows that students perform best when matching the teacher's architecture (diagonal), with performance degrading as the capacity gap widens as expected.
...
\begin{table}[H]
    ...
    \end{tabular}
    \label{ch4:tab:abla-crossarch}
\end{table}


\subsubsection{Students of compressed capacity} \label{ch4:sssec:04-experiments-ablation-compress}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.76\columnwidth]{figures/40-research02/44-research02-experiments-abla-compression.pdf}
    \caption{Distillation accuracy (\%) by compression ratio (CR).}
    \label{ch4:fig:abla-compression}
\end{figure}
