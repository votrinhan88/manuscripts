...
\begin{figure}[t] % fig:03-framework-teaser
    \centering
    \includegraphics[width=1.00\textwidth]{figures/40-research02/43-research02-framework-teaser.pdf}
    \caption{
        ...
        \textbf{(2) Contrast:}
            We construct the dataset $\mathcal{D} := \left\{x | x\sim\mathcal{X}_{\mathcal{P}}\right\}$ and train a primer student $S_0$ as a feature extractor.
    ...
    \label{ch4:fig:03-framework-teaser}
\end{figure}

\subsection{Problem setup} \label{ch4:ssec:03-framework-setup}
...
\begin{foldable} % Naive solution
    ...
    Without real data, this necessitates using uniform noise images, e.g., $x\sim\mathcal{X}_{\mathcal{U}}$, where $\mathcal{X}_{\mathcal{U}}=\mathcal{U}\left[0,1\right]^{C\times H\times W}$.
    In this restricted interface, only the hard label $\hat{y}_{T}^\text{hard}=T(x)\in\left\{ 1,\ldots,K\right\}$ is available to guide the student's probabilistic outputs $\hat{y}_{S}^\text{soft}=S\left(x\right)$.
    The objective is defined as:
    \begin{equation} \label{ch4:eq:loss-naive-kd}
...
\begin{foldable} % Proposal
    \textbf{Proposal.}
    To improve BBDFKD, we propose \textit{Diverse Image Priors Knowledge Distillation} (\textbf{DIPKD}), structured into three collaborative phases:
    \begin{enumerate}
        \item
        ...
        \item
            ...
            We further train a primer student $S_0$ to optimize $\mathcal{D}$ via a contrastive learning objective to make every sample informationally \textit{distinguishable}.
        \item
            Finally, in the \textbf{Distillation} phase, we utilize the optimized dataset $\mathcal{D}$ to distill the student $S$.
            Crucially, we extract soft knowledge from the primer student $S_0$ to restore the inter-class signals absent in previous BBDFKD methodologies.
    \end{enumerate}
\end{foldable}



\subsection{Synthesis: Reconstructing the knowledge domain} \label{ch4:ssec:03-framework-synthesis}
...
\begin{figure*}[t] % fig:03-framework-viz-synthesis
    ...
    \caption{
        ...
        \textbf{(1)} Hierarchical noise: We sample $x_{\text{hn}}$ from multi-scale noise images to capture hierarchical structures;
    ...
    \label{ch4:fig:03-framework-viz-synthesis}
\end{figure*}

\begin{foldable} % Motivation --> overview of 3 components
    Natural images are characterized by strong local pixel structures and complex, nonlinear semantics~\cite{lecun1998gradient}.
    To capture these properties, we generate image priors $\mathcal{X}_\mathcal{P}$ through a pipeline of three stages: \textit{hierarchical noise}, \textit{nonlinear transformation}, and \textit{semantic cutmixing}, visualized in Fig.~\ref{ch4:fig:03-framework-viz-synthesis}.
...
\subsubsection{Hierarchical noise} \label{ch4:sssec:03-framework-synthesis-hiernoise}
...
\begin{foldable} % Sampling multi-scale noise images
    In a standard pixel-wise noise image $x\sim\mathcal{X_{\mathcal{U}}}$, patterns are primarily local and independent.
    ...
    \begin{equation} \label{ch4:eq:synthesis-hiernoise-multiscale}
        \left\{ x_{d}|x_{d}\sim\mathcal{U}\left[0,1\right]^{C\times2^{d}\times2^{d}}\right\} _{d=0}^{d_{\text{max}}},
    \end{equation}
    where $d_{\text{max}}=\text{ceil}\left(\log_{2}\max\left(H,W\right)\right)$ is the first scale to sufficiently cover $H\times W$.
    The dimensions are derived deterministically, e.g., a $32 \times 32$ image is generated from scales $\{2^0, 2^1, \ldots, 2^5\}$.
    ...
    We then mix the noise images $\left\{ x_{d}\right\}$ to obtain the \textit{hierarchical noise} image:
    \begin{equation} \label{ch4:eq:synthesis-hiernoise-combine}
...
\subsubsection{Nonlinear transformation} \label{ch4:sssec:03-framework-synthesis-nonlintrans}
\begin{foldable} % Nonlinear transformation
    ...
    \begin{equation} \label{ch4:eq:synthesis-nonlintrans}
        x_{\text{nt}}=f_{\text{crop}}^{H\times W} \circ f_{\text{elas}} \circ f_{\text{rot}}\left(x_{\text{hn}}\right).
    \end{equation}
\end{foldable}
...
\subsubsection{Semantic cutmixing} \label{ch4:sssec:03-framework-synthesis-semcutmix}
...
\begin{foldable} % Design: diverging filter, mask, and cutmix
    We achieve these patterns through refinement of a semantic mask $\hat{m}$, initialized as $m_{0} \sim \mathcal{U}\left[0,1\right]^{1 \times H \times W}$.
    ...
    \begin{equation} \label{ch4:eq:synthesis-diverging-filter}
        f_{\text{divg}}\left(m\right)=\begin{cases}
            2 m^{2}           & , m \ge 0.5 \\
            1 - 2 (m - 1)^{2} & , m < 0.5
        \end{cases}.
    \end{equation}
...
\begin{foldable} % Steady-state convergence and Diversity
    We obtain the final $\hat{m}$ by iteratively applying $m_{i+1}=f_{\text{divg}} \circ f_{\text{blur}}\left(m_{i}\right)$ until the mean absolute pixel-wise update reaches a steady-state threshold $\epsilon < 10^{-2}$, empirically around 10 iterations.
    This mask is used for pairwise cutmixing $\left\{ x_{\text{nt}}\right\}$ against themselves or random monochromatic images $\mathcal{X}_{\text{mc}}$ to form our final \textbf{image~priors} $\mathcal{X}_\mathcal{P} := \{x\}$, where:
    \begin{equation} \label{ch4:eq:synthesis-semcutmix}
        x=\text{CutMix}\left(x_{i},x_{j};\hat{m}\right), \text{\quad s.t. \quad} x_{i},x_{j} \sim \left\{ x_{\text{nt}}\right\} \cup \mathcal{X}_{\text{\text{mc}}} \text{ and } x_{i} \ne x_{j}.
    \end{equation}
...
\subsection{Contrast: Collaborative diversity optimization} \label{ch4:ssec:03-framework-contrast}
\begin{foldable} % Primer student S_0
    ...
    We propose the novel use of a \textbf{primer student} $S_{0}$ as a white-box mediator to bridge this informational gap.
    We first train $S_{0}$ on an image priors dataset $\mathcal{D} = \left\{ x | x \sim \mathcal{X}_{\mathcal{P}} \right\}$ following the Hard-KD objective:
    \begin{equation} \label{ch4:eq:loss-hard-kd}
    ...
    \end{equation}
    where $\hat{y}_{S_0}^\text{soft} = S_0(x)$ is its probability prediction, and $\hat{y}_{T}^\text{hard}=T\left(x\right)$ is the teacher's hard label.
...
\begin{foldable} % Embeddings & cosine similarity
    Following~\cite{fang2021contrastive}, diversity correlates with \textit{``how distinguishable''} samples are, under an \textit{explicitly optimizable objective}.
...
\begin{foldable} % Contrastive loss
    ...
    \begin{equation} \label{ch4:eq:loss-contrastive}
...
\subsection{Distillation: Restoring informative signals} \label{ch4:ssec:03-framework-distillation}
\begin{foldable}
    Once the synthetic dataset $\mathcal{D}$ has been optimized for diversity, we use it for the final training of the student $S$.
    Our objective is to restore the high-bandwidth signals typical of standard KD~\cite{hinton2015distilling}, i.e., incorporating both \textbf{hard} and \textbf{soft knowledge}, which were \textit{limited or entirely absent} in recent works~\cite{wang2021zero, zhang2022ideal, yuan2024data}.
    Having privileged access to the primer network $S_0$, we form an optimal distillation objective by jointly matching the student's logits $S^{\mathcal{Z}}\left(x\right)$ with soft labels $S_{0}^{\mathcal{Z}}\left(x\right)$ and hard labels $\hat{y}_{T}^\text{hard}$:
    \begin{align} \label{ch4:eq:loss-kd-total}
        \mathcal{L}_{S}
